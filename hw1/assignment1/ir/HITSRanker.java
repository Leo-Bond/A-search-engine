...
public class HITSRanker {
    ...
    public HITSRanker( String linksFilename, String titlesFilename, Index index ) {
        ...
        for (Map.Entry<String, Integer> entry : titleToId.entrySet()) {
            idToTitle.put(entry.getValue(), entry.getKey());
        }
    }
...
//    }
    ...
    private void iterate1(String[] titles) {
    ...
    private void iterate(String[] titles) {
        ...
        for (String title : titles) {
            Integer node = titleToId.get(title);
            if (node != null) {
                baseSet.add(node);
                rootSet.add(node);
            }
        }
//        System.err.println("Base set initial size: " + baseSet.size());

        for (String title : titles) {
            Integer node = titleToId.get(title);
            if (node != null && link.containsKey(node)) {
                for (Integer outNode : link.get(node).keySet()) {
                    baseSet.add(outNode);
                }
            }
        }

//        System.err.println("Base set 2 size: " + baseSet.size());

        for (Integer fromNode : link.keySet()) {
            HashMap<Integer, Boolean> outlinks = link.get(fromNode);
            for (Integer outNode : outlinks.keySet()) {
                if (rootSet.contains(outNode)) {
                    baseSet.add(fromNode);
                }
            }
        }

        List<Integer> baseNodes = new ArrayList<>(baseSet);
        System.err.println("Base node size: " + baseNodes.size());

        hubs = new HashMap<>();
        authorities = new HashMap<>();
        for (Integer node : baseNodes) {
            hubs.put(node, 1.0);
        ...
        for (int step = 0; step < MAX_NUMBER_OF_STEPS; step++) {
            ...
            for (Integer i : baseNodes) {
                ...
                if (link.containsKey(i)) {
                    for (Integer k : link.get(i).keySet()) {
            ...
            if (normA != 0) {
                for (Integer node : baseNodes) {
                    newAuthorities.put(node, newAuthorities.get(node) / normA);
                }
            }
            ...
            if (normH != 0) {
                for (Integer node : baseNodes) {
                    newHubs.put(node, newHubs.get(node) / normH);
                }
            }

            double diff = 0.0;
            for (Integer node : baseNodes) {
                diff += Math.abs(newAuthorities.get(node) - authorities.get(node));
                diff += Math.abs(newHubs.get(node) - hubs.get(node));
            }
            ...
            if (diff < EPSILON) {
                System.err.println("HITS converged after " + (step + 1) + " iterations on base set.");
                break;
            }
        }
    }
    ...
    PostingsList rank(PostingsList post) {
        ...
        for (Map.Entry<Integer, Double> entry : authorities.entrySet()) {
            ...
            Integer docID = index.docIDs.get(prefix + idToTitle.get(id));
            Double score = hubs.get(id) + authorities.get(id);
            combined.put(docID, score);
        }
        HashMap<Integer, Double> sortedCombined = sortHashMapByValue(combined);
        PostingsList result = new PostingsList();
        for (Map.Entry<Integer, Double> entry : sortedCombined.entrySet()) {
            PostingsEntry newEntry = new PostingsEntry(entry.getKey(), 0, entry.getValue());
    ...
    public double getHitsRank(String fileName) {
        Integer docName = titleToId.get(fileName);
        Double hubScore = hubs.get(docName);
        Double authorityScore = authorities.get(docName);

        double alpha = 0.5;
        double HitsScore = alpha * authorityScore + (1 - alpha) * hubScore;
